...
public class Matrix
{
    ...
    {
        var result = from rowsM1 in m2.values.Select((v, index) => new { Value = v, Index = index })
                     join rowsM2 in m1.values.Select((v, index) => new { Value = v, Index = index }) on rowsM1.Index equals rowsM2.Index
                     select rowsM1.Value.Select((r, index) => r + rowsM2.Value.ElementAt(index));
        return new Matrix(result.Select((rows) => rows.ToArray()).ToArray());
    }
}
